  ...
  if (!me || !can("user.read")) {
    return (
      <div className="p-6 space-y-3">
        ...
        <p className="text-sm text-zinc-600">Tu usuario no tiene permisos para acceder al panel.</p>
        <Link className="text-emerald-700" href="/">Volver al inicio</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <header className="fixed inset-x-0 top-0 z-50 h-14 border-b bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/70">
        <div className="h-full px-4 flex items-center gap-3">
          ...
          <Link href="/admin" className="font-semibold tracking-tight">Panel Admin</Link>
          <span className="ml-2 rounded-full bg-emerald-50 text-emerald-700 text-xs px-2 py-0.5">Farmacia</span>
          ...
          <div className="hidden md:flex items-center gap-2 ml-6 flex-1 max-w-2xl rounded-lg border bg-white px-3 py-1.5 text-sm focus-within:ring-1 focus-within:ring-emerald-500">
            ...
            <input className="w-full outline-none placeholder:text-zinc-400" placeholder="Buscar (usuarios, pedidos, productos…)" />
          </div>
          ...
          <div className="ml-auto relative">
            ...
            >
              ...
              <svg className="h-4 w-4 text-zinc-500" viewBox="0 0 20 20" fill="currentColor"><path d="M5 7l5 5 5-5" /></svg>
            </button>

            {menuOpen && (
              <div role="menu" className="absolute right-0 mt-2 w-48 rounded-xl border bg-white shadow-xl ring-1 ring-black/5 py-1 text-sm z-50">
                <Link href="/" className="block px-3 py-2 hover:bg-zinc-50" onClick={() => setMenuOpen(false)}>Ver sitio</Link>
                <button
      ...
      <main className="pt-14 md:pl-72 p-4 sm:p-5">
        {children}
      </main>

      {/* Sidebar móvil (overlay) */}
      <div className={`fixed inset-0 z-50 md:hidden transition ${openMobile ? "pointer-events-auto" : "pointer-events-none"}`}>
        <div

...
type ProductoApi = Omit<Producto, "stockActual"> & {
...
export default function ProductosAdmin() {
  ...
  const [newUnidadData, setNewUnidadData] = useState({
    codigo: "",
    nombre: "",
  });
  ...
  const createNewCategoria = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData((prev) => ({
          ...prev,
          categoriaId: newCategoria.id.toString(),
        }));
        setNewCategoriaName("");
  ...
  const uploadImage = async (file: File) => {
    try {
      const response = await fetch(
        `/api/productos/presign?filename=${encodeURIComponent(
          file.name
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        !editingProduct &&
        initialLotEnabled &&
        initialLot.cantidad > 0 &&
        initialLot.fechaVenc
      ) {
        const loteResponse = await fetch(
          `/api/productos/${savedProducto.id}/lotes`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            credentials: "include",
        ...
        if (!loteResponse.ok) {
          ...
          Swal.fire(
            "Advertencia",
            message || "El lote inicial no pudo registrarse.",
            "warning"
          );
        }
      }
  ...
  const handleCreateLote = async () => {
    ...
    if (lotForm.cantidad <= 0 || !lotForm.fechaVenc) {
      Swal.fire(
        "Atencion",
        "Define cantidad y fecha de vencimiento para el lote.",
        "warning"
      );
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/productos/${editingProduct.id}/lotes`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
                  ...
                  <tr key={producto.id}>
                    ...
                    <td className="px-6 py-4 align-top">
                      <div className="flex items-start gap-2">
                        <div className="text-sm font-medium text-gray-900 leading-5 flex items-center gap-2">
                          {producto.nombre}
                        </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {producto.requiereReceta ? (
                        ...
                        <span className="inline-flex items-center rounded-full bg-gray-50 px-2 py-0.5 text-xs font-semibold text-gray-700 border border-gray-200">
                          No
                        </span>
                      )}
                    </td>
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Se calcula como la suma de los lotes registrados.
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                ...
                <label className="inline-flex items-center gap-2 text-sm text-gray-700">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((prev) => ({
                        ...prev,
                        requiereReceta: e.target.checked,
                      }))
                    }
                  />
                  Requiere receta
                ...
                <div className="space-y-4 rounded border border-gray-200 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className="text-xs text-gray-500">
                      Stock total: {formData.stockActual}
                    </span>
                  </div>

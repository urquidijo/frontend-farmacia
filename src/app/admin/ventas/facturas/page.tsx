...
export default function FacturasAdmin() {
  ...
  const fetchFacturas = async () => {
    try {
      ...
      const response = await fetch(`${backendUrl}/pagos/facturas`, { cache: 'no-store' })
      if (response.ok) {
  ...
  const exportarHTML = () => {
    ...
    const filas = filteredFacturas.map((f, idx) => `
      <tr>
        <td>${idx + 1}</td>
                  ...
                  <tr key={f.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${
                        f.estado === 'PAGADA' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'
                      }`}>
                        {f.estado}
                      </span>
                    </td>
